...
export default function Home() {
  ...
  const activeState = selectedState === "india" ? undefined : getIndiaState(selectedState);
  const areaLabel = getAreaLabel(selectedState, selectedDistrict);
  const areaLocations = useMemo(() => getLocationsForArea(locations, selectedState, selectedDistrict), [locations, selectedState, selectedDistrict]);
  const visibleLocations = useMemo(() => areaLocations.filter((location) => {
  ...
  }), [areaLocations, category, query]);
  const selectedAreaLocation = selectedLocation ? areaLocations.find((location) => location.id === selectedLocation.id) ?? selectedLocation : null;
  const averageWait = Math.round(areaLocations.reduce((total, location) => total + location.wait, 0) / Math.max(areaLocations.length, 1));
  ...
  return <div className="page-in">
    <section className="relative isolate min-h-[520px] overflow-hidden border-b border-white/[.08] lg:min-h-[560px]">
      ...
      <div className="absolute inset-0 -z-10 bg-[linear-gradient(0deg,#0d090f_0%,transparent_35%,rgba(14,18,16,.28)_100%)]" />
        ...
        <div className="max-w-[670px]">
          ...
          <div className="hero-reveal hero-reveal-delay-3 mt-7 max-w-[520px] rounded-2xl border border-white/[.14] bg-[#0d090f]/68 p-3 backdrop-blur-xl">
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-2">
              <label className="sr-only" htmlFor="state-select">Select a state or union territory</label>
              <select id="state-select" value={selectedState} onChange={(event) => handleStateChange(event.target.value)} className="min-w-0 rounded-xl border border-white/[.13] bg-[#1a1018] px-3 py-3 text-[12px] text-[#f3f3e8] outline-none transition focus:border-[#ff8a2b] focus:ring-2 focus:ring-[#ff8a2b]/20"><option value="india">India · All areas</option>{indiaStates.map((state) => <option key={state.id} value={state.id}>{state.name} · {state.kind}</option>)}</select>
              <label className="sr-only" htmlFor="district-select">Select a district</label>
              <select id="district-select" value={selectedDistrict} onChange={(event) => handleDistrictChange(event.target.value)} disabled={selectedState === "india"} className="min-w-0 rounded-xl border border-white/[.13] bg-[#1a1018] px-3 py-3 text-[12px] text-[#f3f3e8] outline-none transition focus:border-[#ff8a2b] focus:ring-2 focus:ring-[#ff8a2b]/20 disabled:cursor-not-allowed disabled:opacity-45"><option value="all">All districts in {activeState?.name ?? "India"}</option>{activeState?.districts.map((district) => <option key={district.id} value={district.id}>{district.name}</option>)}</select>
            </div>
          </div>
        </div>
        <div className="mt-10 grid max-w-[350px] grid-cols-2 gap-2 lg:mb-1 lg:mt-0 lg:min-w-[330px]">
          <div className="rounded-2xl border border-white/[.13] bg-[#0d090f]/55 p-4 backdrop-blur-xl"><p className="eyebrow">Reporting now</p><p className="metric mt-3 text-[32px] font-semibold text-[#f3f3e8]">{areaLocations.length}</p><p className="mt-1 text-[11px] text-[#9ba89d]">places in {selectedState === "india" ? "India" : activeState?.name}</p></div>
          <div className="rounded-2xl border border-white/[.13] bg-[#0d090f]/55 p-4 backdrop-blur-xl"><p className="eyebrow">Your call</p><p className="mt-3 text-[25px] font-semibold leading-7 text-[#ffbd4a]">{averageWait <= 15 ? "Go now" : "Check first"}<span className="text-[#dfe9df]">.</span></p><p className="mt-1 text-[11px] text-[#9ba89d]">{areaLabel} · {averageWait}m average</p></div>
        </div>
      </div>
    </section>

    <section ref={mapRef} className="relative scroll-mt-[70px]">
      <div className="pointer-events-none absolute left-0 right-0 top-0 z-20 flex -translate-y-1/2 justify-center px-5"><div className="pointer-events-auto flex w-full max-w-[760px] flex-col gap-2 rounded-2xl border border-white/[.14] bg-[#111813]/95 p-2 shadow-2xl backdrop-blur-xl sm:flex-row sm:items-center"><div className="flex min-w-0 flex-1 items-center gap-3 rounded-xl bg-white/[.055] px-3.5 py-2.5"><Search size={17} className="shrink-0 text-[#74847a]" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Search in ${areaLabel}`} aria-label={`Search locations in ${areaLabel}`} className="min-w-0 flex-1 bg-transparent text-[13px] text-[#f3f3e8] outline-none placeholder:text-[#74847a]" /></div><div className="flex items-center gap-1 overflow-x-auto px-1 pb-0.5 sm:max-w-[400px]">{categories.slice(0, 5).map((item) => <button key={item} onClick={() => setCategory(item)} className={cn("whitespace-nowrap rounded-lg px-2.5 py-2 text-[11px] font-medium transition", item === category ? "bg-[#ff8a2b] text-[#160b05]" : "text-[#829187] hover:bg-white/[.06] hover:text-[#e8f0e8]")}>{item === "Government" ? "Govt" : item}</button>)}</div></div></div>
      <div className="relative flex min-h-[560px] flex-col lg:min-h-[calc(100vh-70px)]"><MapSurface locations={visibleLocations} selectedId={selectedAreaLocation?.id} areaLabel={areaLabel} onSelect={(location) => selectLocation(location.id)} /><div className="pointer-events-none absolute bottom-5 left-5 z-20 max-w-[300px] rounded-xl border border-white/[.12] bg-[#0d090f]/85 px-4 py-3 backdrop-blur-xl"><p className="text-[12px] font-medium text-[#dbe6dc]">{selectedAreaLocation ? "Tap another dot to compare." : `Tap a dot. We'll tell you if it's worth the trip in ${areaLabel}.`}</p><p className="mt-1 text-[10px] leading-4 text-[#7e8d82]">The map updates as people check in and check out.</p></div></div>
    </section>
    <section className="border-t border-white/[.08] bg-[#120c12] px-5 py-14 sm:px-10 lg:px-12 lg:py-20"><div className="mx-auto max-w-[1280px]"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="eyebrow">Read the signal</p><h2 className="mt-3 max-w-[550px] font-display text-[31px] font-semibold leading-[1.05] tracking-[-.055em]">Places no app tracks yet.</h2><p className="mt-3 max-w-[520px] text-[14px] leading-6 text-[#929f94]">Temples, stalls, small clinics — added by people who were just there.</p></div><Link href="/community" className="flex w-fit items-center gap-2 text-[12px] font-semibold text-[#ff8a2b] hover:text-[#ffbd4a]">Add a place <ArrowRight size={15} /></Link></div><div className="mt-8 grid gap-3 md:grid-cols-3">{areaLocations.slice(0, 3).map((location) => <div key={location.id} className="flex items-center justify-between rounded-2xl border border-white/[.08] bg-[#1d111b] p-4 transition hover:border-white/[.18]"><div className="flex items-center gap-3"><span className="h-2.5 w-2.5 rounded-full" style={{ background: crowdMeta[location.crowd].color, boxShadow: `0 0 16px ${crowdMeta[location.crowd].color}99` }} /><div><p className="text-[13px] font-semibold text-[#dfe8df]">{location.name}</p><p className="mt-1 text-[11px] text-[#78877c]">{location.category} · {location.neighborhood.split(" · ")[0]}</p></div></div><div className="text-right"><p className="metric text-[17px] font-semibold" style={{ color: crowdMeta[location.crowd].color }}>{location.wait}m</p><p className="text-[10px] text-[#758379]">{crowdMeta[location.crowd].label}</p></div></div>)}</div></div></section>
    {selectedAreaLocation && <LocationDetail location={selectedAreaLocation} onClose={() => selectLocation(null)} />}
  </div>;
}
