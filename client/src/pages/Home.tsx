    ...
    <section className="relative isolate min-h-[520px] overflow-hidden border-b border-white/[.08] lg:min-h-[560px]">
      ...
      <div className="mx-auto flex min-h-[520px] max-w-[1280px] flex-col justify-end px-5 pb-14 pt-16 sm:px-10 lg:min-h-[560px] lg:flex-row lg:items-end lg:justify-between lg:gap-10 lg:px-12 lg:pb-16">
        <div className="max-w-[670px]">
          <div className="hero-reveal inline-flex items-center gap-2 rounded-full border border-[#ff8a2b]/25 bg-[#0d090f]/45 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[.16em] text-[#8bd5b5] backdrop-blur"><Sparkles size={12} /> A living city map · Bengaluru</div>
          <h1 className="hero-reveal hero-reveal-delay-1 mt-5 max-w-[660px] font-display text-[45px] font-semibold leading-[.98] tracking-[-.065em] text-[#f3f3e8] sm:text-[62px] lg:text-[72px]">Every queue in your city, <span className="text-[#ffbd4a]">right now.</span></h1>
          ...
          <div className="hero-reveal hero-reveal-delay-3 mt-8 flex flex-wrap items-center gap-3"><button onClick={scrollToMap} className="group flex items-center gap-3 rounded-xl bg-[#f3f3e8] px-4 py-3 text-[13px] font-semibold text-[#0d090f] transition hover:bg-[#dbe8de] active:scale-[.97]">Explore the map <ArrowDownRight size={16} className="transition group-hover:translate-y-0.5 group-hover:translate-x-0.5" /></button><Link href="/plan" className="flex items-center gap-2 rounded-xl border border-white/[.18] bg-[#0d090f]/40 px-4 py-3 text-[13px] font-semibold text-[#e7eee7] backdrop-blur transition hover:border-white/[.35] hover:bg-white/[.08]">Plan my day <ArrowRight size={15} /></Link></div>
        </div>
        <div className="mt-10 grid max-w-[350px] grid-cols-2 gap-2 lg:mb-1 lg:mt-0 lg:min-w-[330px]">
          <div className="rounded-2xl border border-white/[.13] bg-[#0d090f]/55 p-4 backdrop-blur-xl"><p className="eyebrow">Reporting now</p><p className="metric mt-3 text-[32px] font-semibold text-[#f3f3e8]">7</p><p className="mt-1 text-[11px] text-[#9ba89d]">places across the city</p></div>
          <div className="rounded-2xl border border-white/[.13] bg-[#0d090f]/55 p-4 backdrop-blur-xl"><p className="eyebrow">Your call</p><p className="mt-3 text-[25px] font-semibold leading-7 text-[#ffbd4a]">Go now<span className="text-[#dfe9df]">.</span></p><p className="mt-1 text-[11px] text-[#9ba89d]">CityCare OPD is quiet</p></div>
        </div>
      </div>
    </section>
