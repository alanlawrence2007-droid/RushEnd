/* Queueless / Signal Cartography: the preloader is a slow, centered handoff from signal to map. */
import { AnimatePresence, motion } from "framer-motion";
import { ArrowRight, MapPinned } from "lucide-react";
import { useEffect, useState } from "react";

const stairColors = ["#0e1210", "#121a15", "#0e1210", "#152019", "#0e1210", "#121a15"];

export default function QueuelessPreloader({ children }: { children: React.ReactNode }) {
  const [visible, setVisible] = useState(true);
  const [stage, setStage] = useState<"intro" | "cover" | "reveal" | "done">("intro");
  ...
  useEffect(() => {
    if (!visible) return;
    const coverTimer = window.setTimeout(() => setStage("cover"), reducedMotion ? 100 : 2200);
    const revealTimer = window.setTimeout(() => setStage("reveal"), reducedMotion ? 500 : 4700);
    const finishTimer = window.setTimeout(() => setStage("done"), reducedMotion ? 900 : 7000);
    return () => {
  ...
  const showStairs = stage === "cover" || stage === "reveal";
  ...
  return <>
    ...
    <AnimatePresence>
      {visible && <motion.div key="queueless-preloader" initial={{ opacity: 1 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: reducedMotion ? 0 : .45, ease: [0.23, 1, 0.32, 1] }} className="fixed inset-0 z-[100] flex min-h-screen flex-col overflow-hidden bg-[#0e1210] text-[#f3f3e8]" role="status" aria-label="Loading Queueless">
        <div className="pointer-events-none absolute inset-0 opacity-60" aria-hidden="true"><div className="absolute left-[-10%] top-[8%] h-[75vw] w-[75vw] rounded-full border border-[#1d9e75]/10" /><div className="absolute left-[5%] top-[15%] h-[62vw] w-[62vw] rounded-full border border-[#b6cbbd]/[.06]" /><div className="absolute right-[-10%] bottom-[-30%] h-[78vw] w-[78vw] rounded-full border border-[#378add]/[.08]" /></div>
        ...
        <div className="pointer-events-none absolute inset-0 z-30 flex" aria-hidden="true"><div className="flex h-full w-1/2 justify-end overflow-hidden">{stairColors.map((color, index) => <motion.div key={`left-${index}`} initial={{ x: "-105%" }} animate={{ x: showStairs ? (isRevealing ? "-105%" : "0%") : "-105%" }} transition={{ duration, delay: reducedMotion ? 0 : (isRevealing ? stairColors.length - index - 1 : index) * .14, ease: [0.77, 0, 0.175, 1] }} style={{ backgroundColor: color, borderRight: `1px solid ${index % 2 === 0 ? "rgba(29,158,117,.42)" : "rgba(182,203,190,.16)"}` }} className="h-full w-[16.666%]" />)}</div><div className="flex h-full w-1/2 overflow-hidden">{stairColors.map((color, index) => <motion.div key={`right-${index}`} initial={{ x: "105%" }} animate={{ x: showStairs ? (isRevealing ? "105%" : "0%") : "105%" }} transition={{ duration, delay: reducedMotion ? 0 : (isRevealing ? stairColors.length - index - 1 : index) * .14, ease: [0.77, 0, 0.175, 1] }} style={{ backgroundColor: color, borderLeft: `1px solid ${index % 2 === 0 ? "rgba(186,117,23,.42)" : "rgba(182,203,190,.16)"}` }} className="h-full w-[16.666%]" />)}</div></div>
