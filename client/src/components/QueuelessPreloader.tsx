...
export default function QueuelessPreloader({ children }: { children: React.ReactNode }) {
  ...
  const [stairEntering, setStairEntering] = useState(false);
  ...
  useEffect(() => {
    if (!visible) return;
    const enterTimer = window.setTimeout(() => setStairEntering(true), reducedMotion ? 100 : 850);
    const leaveTimer = window.setTimeout(() => setLeaving(true), reducedMotion ? 500 : 2200);
    return () => {
