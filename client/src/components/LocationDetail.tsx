/* Queueless / Signal Cartography: a location opens as a verdict-led drawer, not a passive data card. */
...
function QueuePeople({ count, level }: { count: number; level: QueueLocation["crowd"] }) {
  ...
  return <div className="flex min-h-[48px] flex-wrap items-center gap-2 rounded-xl border border-white/[.08] bg-[#101611] px-3 py-3">
    {Array.from({ length: visible }).map((_, index) => <span key={index} className={`queue-dot ${index % 4 === 1 ? "delay-1" : index % 4 === 2 ? "delay-2" : index % 4 === 3 ? "delay-3" : ""} flex h-5 w-5 items-center justify-center rounded-full border border-[#0e1210] text-[8px] font-bold ${level === "low" ? "bg-[#1d9e75] text-[#07120e]" : level === "moderate" ? "bg-[#ba7517] text-[#180e04]" : "bg-[#d85a30] text-[#180b07]"}`}><Users size={10} strokeWidth={2.4} /></span>)}
    ...
    <aside className="panel-in fixed bottom-0 right-0 z-50 flex max-h-[88vh] w-full flex-col overflow-y-auto border-t border-white/[.12] bg-[#121914] shadow-[-24px_0_80px_rgba(0,0,0,.36)] sm:max-w-[480px] lg:bottom-0 lg:top-[70px] lg:max-h-none lg:w-[430px] lg:border-l lg:border-t-0" aria-label={`${location.name} details`}>
      ...
      <div className="space-y-6 px-6 py-6">
        <div className="rounded-2xl border border-white/[.09] bg-[#18211b] p-5">
          <div className="flex items-center gap-2"><span className={`h-2 w-2 rounded-full ${location.crowd === "low" ? "bg-[#1d9e75] shadow-[0_0_9px_rgba(29,158,117,.9)]" : location.crowd === "moderate" ? "bg-[#ba7517] shadow-[0_0_9px_rgba(186,117,23,.7)]" : "bg-[#d85a30] shadow-[0_0_9px_rgba(216,90,48,.7)]"}`} /><span className={`text-[11px] font-semibold uppercase tracking-[.13em] ${meta.className}`}>{meta.label} crowd</span></div>
          ...
          <div className="mt-5 flex items-center gap-2 text-[10px] text-[#8d9b90]"><Radio size={13} className={location.confidence === "live" ? "text-[#1d9e75]" : "text-[#ba7517]"} /> {location.confidence === "live" ? `Live — updated ${location.lastUpdated}` : "Estimated from past patterns"}</div>
        </div>
        ...
        <div className="space-y-2">
          {!isCheckedIn ? <Button onClick={() => checkIn(location.id)} className="h-11 w-full justify-center gap-2 rounded-xl bg-[#378add] text-[13px] font-semibold text-[#06101b] shadow-[0_8px_25px_rgba(55,138,221,.18)] hover:bg-[#65a6ee]"><LogIn size={16} /> I'm here — check in</Button> : <Button onClick={() => checkOut(location.id)} className="h-11 w-full justify-center gap-2 rounded-xl bg-[#1d9e75] text-[13px] font-semibold text-[#07120e] shadow-[0_8px_25px_rgba(29,158,117,.18)] hover:bg-[#4cc99b]"><Check size={16} /> Checked in · Just finished? Check out</Button>}
