/* Queueless / Signal Cartography: the map surface is a living civic canvas, not a dashboard background. */
  ...
  return <div className="map-texture relative min-h-[520px] flex-1 overflow-hidden border-y border-white/[.08] lg:min-h-[calc(100vh-70px)] lg:border-l">
    <div className="pointer-events-none absolute inset-0 opacity-70">
      <svg viewBox="0 0 1000 680" preserveAspectRatio="none" className="h-full w-full">
        ...
        <path d="M325 362 C380 295 446 300 518 355 S650 430 755 374" fill="none" stroke="#378add" strokeWidth="3" strokeLinecap="round" strokeDasharray="3 11" className="route-line" opacity=".7" />
      </svg>
    </div>
    ...
    <div className="pointer-events-none absolute left-5 top-5 z-10 flex items-center gap-2 rounded-full border border-white/[.12] bg-[#0e1210]/80 px-3 py-2 text-[11px] text-[#a8b5aa] backdrop-blur"><span className="h-1.5 w-1.5 rounded-full bg-[#1d9e75] shadow-[0_0_8px_rgba(29,158,117,.8)]" /> 7 places reporting now <span className="text-[#56635a]">·</span> <span className="metric">34</span> min avg wait</div>
    ...
    {locations.map((location, index) => {
      ...
      return <button key={location.id} onClick={() => onSelect(location)} className="group absolute z-10 -translate-x-1/2 -translate-y-1/2 text-left" style={{ left: `${location.position.left}%`, top: `${location.position.top}%` }} aria-label={`${location.name}, ${meta.label}, ${location.wait} minute wait`}>
        <span className={cn("signal-halo absolute left-1/2 top-1/2 h-12 w-12 -translate-x-1/2 -translate-y-1/2 rounded-full border", location.crowd === "low" ? "border-[#1d9e75]/50" : location.crowd === "moderate" ? "border-[#ba7517]/50" : "border-[#d85a30]/50", index % 3 === 1 ? "slow" : index % 3 === 2 ? "fast" : "")} />
    ...
    <div className="absolute z-20 -translate-x-1/2 -translate-y-1/2" style={{ left: `${userLocation.left}%`, top: `${userLocation.top}%` }}>
      <span className="absolute left-1/2 top-1/2 h-9 w-9 -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#378add]/50 bg-[#378add]/10" />
      <span className="relative flex h-4 w-4 items-center justify-center rounded-full border-2 border-[#d8ecff] bg-[#378add] shadow-[0_0_20px_rgba(55,138,221,.7)]"><span className="h-1 w-1 rounded-full bg-white" /></span>
      <span className="absolute left-6 top-1/2 -translate-y-1/2 whitespace-nowrap rounded-md bg-[#378add]/15 px-2 py-1 text-[10px] font-medium text-[#9cc6f5]">You are here</span>
    </div>

    <div className="pointer-events-none absolute bottom-5 left-5 right-5 z-10 flex items-end justify-between gap-4">
      <div className="hidden items-center gap-3 rounded-lg border border-white/[.09] bg-[#0e1210]/75 px-3 py-2 text-[10px] text-[#849187] backdrop-blur sm:flex"><span className="font-semibold uppercase tracking-[.14em] text-[#67766b]">Crowd</span><span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-[#1d9e75]" /> Quiet</span><span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-[#ba7517]" /> Moderate</span><span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-[#d85a30]" /> Heavy</span></div>
      <div className="ml-auto flex items-center gap-2 rounded-lg border border-[#378add]/20 bg-[#0e1210]/80 px-3 py-2 text-[10px] text-[#71aef0] backdrop-blur"><LocateFixed size={12} /> Approx. location</div>
    </div>
  </div>;
}
